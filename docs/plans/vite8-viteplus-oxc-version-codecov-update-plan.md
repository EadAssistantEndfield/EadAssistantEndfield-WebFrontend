...
- `src/features/blueprint/domain/`：蓝图领域解析、路径、布局、模板和建筑目录
- `src/features/blueprint/i18n/`：界面与建筑文案
- `src/features/blueprint/services/`：分享码查询相关外部边界
- `tests/features/blueprint/`：按 feature 镜像的单元测试
- `data/`：蓝图样例、模板和 GameKee 建筑数据
- `scripts/`：i18n 检查、数据抓取、版本号脚本
- `docs/plans/`：迁移计划文档

这个结构短期可维护，但已经出现三个结构压力点：

- `src/features/blueprint/domain/blueprintDomain.ts` 仍承载较多传送带方向推断逻辑，后续可在有回归用例保护时继续拆分。
- `src/features/blueprint/composables/useShareQuerySession.ts` 已隔离分享码副作用，但仍可进一步拆出纯 API client。
  ...
  src/
    ...
    features/
      blueprint/
        ...
        domain/                  # 蓝图布局、路径、模板、建筑目录等纯逻辑
        index.ts                 # public API，等 feature 目录稳定后再补
      share-query/
        services/                # sessionClient / cookieStore / qrcode 等副作用逻辑
        composables/             # useShareCodeQuery 等 UI 状态编排
    i18n/
    style.css
    types.ts
  tests/
...
一次性改造中只落地最有收益的结构动作：`fetch_gamekee_data.py` 迁入 `scripts/data/`，版本脚本放入 `scripts/release/`，并拆分 `useBlueprintParser.ts` 中的副作用逻辑。`features/blueprint`、测试目录镜像和 `docs/architecture.md` 作为后续自然演进目标，不在本次强制搬迁所有组件和 utils。
...
- 修复 `tests/features/blueprint/domain/blueprintSummaryBuilder.test.ts`，让预览图片断言匹配新的本地 GameKee 资源行为。
- 运行：
  - `npm run lint`
  - `npm run lint:style`
  - `npm run check:i18n`
